...
void camera_thread(const int &delay)
{
    // ZMQ socket - image
    zmq::context_t context(1);
    zmq::socket_t socket(context, ZMQ_PUB);
    std::string send_to_address = "tcp://" + MAC + ":5555";
    ...
    cv::Mat current_frame, prev_frame, send_frame;
    std::vector<cv::Point2f> prev_keypoints, current_keypoints;
    ...
    while (!stop_threads)
    {
        ...
        send_frame = current_frame.clone();
        if (!current_frame.empty())
        {
            cv::cvtColor(current_frame, current_frame, cv::COLOR_BGR2GRAY); // Convert to grayscale
        }

        // sending image
        std::vector<int> encode_param = {cv::IMWRITE_JPEG_QUALITY, 80};
        std::vector<uchar> buffer;
        cv::imencode(".jpg", send_frame, buffer, encode_param);
        std::string jpg_as_text = base64_encode(buffer.data(), buffer.size());
        zmq::message_t request(jpg_as_text.size());
        memcpy(request.data(), jpg_as_text.data(), jpg_as_text.size());
        socket.send(request, zmq::send_flags::none);
        std::this_thread::sleep_for(std::chrono::milliseconds(delay));
        auto end = std::chrono::system_clock::now();
        dt = std::chrono::duration_cast<std::chrono::duration<double>>(end - now).count();
    }
}
